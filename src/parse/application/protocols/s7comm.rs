...
impl<'a> S7CommPacket<'a> {
    ...
    pub fn try_from(packet: &'a [u8]) -> Result<Self, &'static str> {
        ...
        if s7_start + 10 > packet.len() {
            println!("Packet too short for S7 header: need {} bytes, have {}", s7_start + 10, packet.len());
        ...
        println!("  TPKT: {:02x} {:02x} {:02x}{:02x}", 
                packet[0], packet[1], packet[2], packet[3]);
        println!("  COTP: {:02x} {:02x} {:02x}", 
        ...
        let parameter = if s7_header.parameter_length > 0 {
            ...
            if param_end > packet.len() {
                println!("Invalid parameter length: param_end={}, packet_len={}", 
                        param_end, packet.len());
    ...
    fn parse_parameter(data: &'a [u8]) -> Result<S7Parameter<'a>, &'static str> {
        ...
        for i in 0..item_count {
            ...
            println!("  Item {}: spec_type={:02x}, length={}", i, spec_type, length);
            
            if offset + 2 + length > data.len() {
                println!("  Invalid item: offset={}, length={}, data_len={}", 
